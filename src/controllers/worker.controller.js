...
export const submitTask = async (req, res) => {
    try {
        const { taskId, title, workerUid, submissionDetails, buyerId, currentDate } = req.body;

        if(!taskId || !title || !workerUid || !submissionDetails || !buyerId || !currentDate) {
...
export const getWorkerSubmissions = async (req, res) => {
    try {
        ...
        const submissions = await Submission.find({ workerInfo: worker._id }).select(
            "taskTitle buyerName status submittedAt"
        );
        ...
        return res.status(500).json({
            message: "Server error. Unable to fetch submissions. Please try again later.",
        });
    }
};
